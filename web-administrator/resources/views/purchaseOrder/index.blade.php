      ...
      <div class="card" style="width: 77%; height:fit-content">
        <div class="card-body">
          <div class="bs-stepper-content">
            <form id="purchase-detail-form" action="{{ route('purchaseOrder.store') }}" method="POST">
              @csrf
              <!-- purchase info -->
              <div id="info-part" class="content" role="tabpanel" aria-labelledby="info-part-trigger">
                <div class="mb-3">
                  ...
                  <select id="supplier" class="form-control select" name="supplier_id" required>
                    <option>--select supplier--</option>
                    @foreach($data as $item)
                    <option value="{{ $item->id }}" data-address="{{ $item->address }}" data-phone="{{ $item->phoneNum }}" data-email="{{ $item->email }}" data-pic-phone="{{ $item->PIC_phone }}" data-pic-name="{{ $item->PIC_name }}" data-alias="{{ $item->alias }}">
                      {{ $item->PIC_name }} - {{ $item->address }}
                    </option>
                    @endforeach
                  </select>
                ...
                <div id="supplier-details" style=" display: none;">
                  <div class="m-3">
                    ...
                    <p class="my-0" id="supplier-address"></p>
                    <p class="my-0" id="supplier-phone"></p>
                    <p class="my-0" id="supplier-email"></p>
                    <p class="my-0" id="PIC-name"></p>
                    <p class="my-0" id="PIC-phone"></p>
                  </div>
                </div>
                <div class="mb-3">
                  <label for="po-number" class="form-label">PO Number:</label>
                  <span class="text-danger font-weight-bold">*</span>
                  <input type="text" id="po-number" class="form-control" name="po_number" placeholder="Purchase Order Number">
                ...
                <div class="mb-3">
                  ...
                  <span class="text-danger font-weight-bold">*</span>
                  <input type="date" id="po-date" class="form-control" name="po_date">
                  ...
                  <table class="table table-hover mt-3">
                    ...
                    <tfoot style="border: none;">
                      <tr>
                        ...
                        <th style="width:20%; border: none" id="total-amount" class="text-right">Rp 0</th>
                      </tr>
                      <tr>
                        <th style="border: none;" colspan="4" class="text-right">PPN 11% :</th>
                        <th style="width:20%; border: none" id="ppn" class="text-right">Rp 0</th>
                        <th style=" border: none">
                          <div class="switch">
                            <input type="hidden" value="0">
                            <input id="tax" type="checkbox" class="form-check-input setting-input" value="1">
                      ...
                      <tr>
                        ...
                        <th style="width:20%; border: none" id="total-ppn" class="text-right">Rp 0</th>
                      </tr>
                    </tfoot>
                  </table>
                </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleSupplierChange() {
      ...
      if (selectedOption.value !== "--select supplier--") {
        const details = {
        ...
        updateSupplierDetails(details);
        generatePONumber(details);
      } else {
        hideSupplierDetails();
        document.getElementById('po-number').value = null;
        document.getElementById('po-date').valueAsDate = null;
      }
    }

    function updateSupplierDetails(details) {
      document.getElementById('supplier-address').innerText = `Address: ${details.address}`;
      document.getElementById('supplier-phone').innerText = `Phone: ${details.phone}`;
      document.getElementById('supplier-email').innerText = `Email: ${details.email}`;
      document.getElementById('PIC-name').innerText = `PIC Name: ${details.picName}`;
      document.getElementById('PIC-phone').innerText = `PIC Phone: ${details.picPhone}`;
      document.getElementById('supplier-details').style.display = 'block';
    }

    function generatePONumber(details) {
      fetch("{{ route('getPrefix') }}") // Memanggil endpoint getPrefix
        .then(response => response.json()) // Mengubah respons menjadi JSON
        .then(data => {
          const prefix = data.prefix;
          const formattedCounter = String(prefix.counter).padStart(prefix.length, '0');
          const now = new Date();
          const month = String(now.getMonth() + 1).padStart(2, '0'); // getMonth() returns 0-11, so we add 1
          const year = String(now.getFullYear()).slice(-2); // get last two digits of the year
          const yymm = year + month;
          const poNumber = `${prefix.prefix}/${yymm}${formattedCounter}/${details.picAlias}`;

          document.getElementById('po-number').value = poNumber;
          document.getElementById('poNumberr').textContent = `PO Number: ${poNumber}`;
          console.log('Prefix data:', prefix, formattedCounter, poNumber);
        })
        .catch(error => {
      ...
      document.getElementById('showPOdate').textContent = `PO Date: ${poDate}`;
    }

    function hideSupplierDetails() {
      document.getElementById('supplier-details').style.display = 'none';
    }
    ...
    window.navigateToDetails = function(event) {
      ...
      // Validasi PO Number
      if (existingPONumbers.includes(poNumber) || poNumber === "") {
        poNumberElement.classList.add('is-invalid');
        isValid = false;
      } else {
        poNumberElement.classList.remove('is-invalid');
      }
    ...
    $('#purchase-detail-form').on('submit', function(event) {
      event.preventDefault();
      const poNumber = document.getElementById('po-number').value;
      document.getElementById('successPO').textContent = "Purchase Order " + poNumber + " Created Successfully";
